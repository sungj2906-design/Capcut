...
export default function EbookReader({ pages, theme, onAdminClick }: Props) {
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center p-4 sm:p-8 overflow-hidden transition-colors duration-300" style={{ backgroundColor: theme.backgroundColor, color: theme.textColor }}>
      {/* Watermark */}
      ...
      {/* Header */}
      <header className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center z-30 border-b border-white/10 backdrop-blur-md transition-colors duration-300" style={{ backgroundColor: `${theme.cardColor}CC` }}>
        <div className="flex items-center gap-2" style={{ color: theme.accentColor }}>
          <BookOpen className="w-6 h-6" />
          <h1 className="font-bold tracking-widest uppercase text-sm md:text-base hidden sm:block">{theme.title}</h1>
        </div>
        ...
        <div className="flex-1 max-w-md mx-4 relative">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full border border-white/10 rounded-full pl-10 pr-4 py-2 text-sm focus:outline-none font-mono transition-colors duration-300"
      ...
      {/* Book Page Container */}
      <div className="relative z-10 w-full max-w-7xl h-[80vh] rounded-2xl shadow-2xl border border-white/5 flex flex-col overflow-hidden mt-16 transition-colors duration-300" style={{ backgroundColor: theme.cardColor }}>
        
        {/* Page Content */}
        <div className={`flex-1 overflow-auto flex custom-scrollbar ${theme.layout === 'vertical' ? 'flex-col' : 'flex-row snap-x snap-mandatory overflow-y-hidden'}`}>
          
          {/* Left Page: Text */}
          <div className={`shrink-0 p-8 md:p-12 overflow-y-auto custom-scrollbar ${theme.layout === 'vertical' ? 'w-full h-auto min-h-[50%]' : 'w-full h-full snap-center'}`}>
            <div className="max-w-2xl mx-auto relative">
              <div className="flex justify-between items-start mb-8">
                <h2 className="text-3xl md:text-5xl font-black tracking-tight" style={{ color: theme.textColor }}>
                  {page.title}
                </h2>
                <button 
                  onClick={toggleBookmark}
                  className={`p-2 rounded-full transition-colors ${bookmarks.includes(currentPage) ? 'bg-white/10' : 'hover:bg-white/5 opacity-50 hover:opacity-100'}`}
              ...
              <div className="prose prose-lg max-w-none" style={{ color: theme.textColor }}>
                {page.content.split('\n').map((paragraph, idx) => (
          ...
          {page.imageUrls && page.imageUrls.filter(u => u.trim() !== '').length > 0 && (
            <div className={`shrink-0 p-8 md:p-12 overflow-hidden bg-black/20 flex flex-col items-center justify-center relative ${theme.layout === 'vertical' ? 'w-full h-[60vh] border-t border-white/5' : 'w-full h-full snap-center border-l border-white/5'}`}>
              <div className="relative w-full h-full flex flex-col">
                <div className="absolute top-0 right-0 flex gap-2 z-20 p-2 rounded-lg backdrop-blur-sm border border-white/10" style={{ backgroundColor: `${theme.cardColor}CC` }}>
                ...
                <div className="flex-1 w-full overflow-x-auto overflow-y-hidden flex snap-x snap-mandatory custom-scrollbar">
                  {page.imageUrls.filter(u => u.trim() !== '').map((url, idx, arr) => (
                    <div key={idx} className="w-full h-full shrink-0 snap-center flex items-center justify-center relative overflow-auto custom-scrollbar">
                      {arr.length > 1 && (
                        <div className="absolute top-4 left-4 text-xs px-3 py-1.5 rounded-full backdrop-blur z-10 font-mono border border-white/10" style={{ backgroundColor: `${theme.cardColor}CC`, color: theme.textColor }}>
                          {idx + 1} / {arr.length}
                        </div>
                      )}
                      <img 
                ...
                {page.imageUrls.filter(u => u.trim() !== '').length > 1 && (
                  <div className="text-center text-xs mt-4 font-mono shrink-0 opacity-50" style={{ color: theme.textColor }}>
        ...
        <div className="h-20 border-t border-white/5 flex items-center justify-between px-8 shrink-0 bg-black/10">
          <button 
            onClick={handlePrev}
            disabled={currentPage === 1}
            className="flex items-center gap-2 disabled:opacity-30 transition-opacity opacity-70 hover:opacity-100"
            style={{ color: theme.textColor }}
          >
            <ChevronLeft className="w-6 h-6" />
            <span className="hidden sm:inline font-mono text-sm uppercase tracking-wider">Prev</span>
          </button>

          <div className="flex items-center gap-4">
            <span className="font-mono" style={{ color: theme.accentColor }}>
              {currentPage} <span className="opacity-50" style={{ color: theme.textColor }}>/ {pages.length}</span>
            </span>
            <form onSubmit={handleJump} className="hidden sm:flex items-center gap-2">
              <input 
                type="text" 
                value={jumpTo}
                onChange={(e) => setJumpTo(e.target.value)}
                placeholder="Jump..."
                className="w-20 border border-white/10 rounded px-2 py-1 text-sm font-mono text-center focus:outline-none transition-colors duration-300"
          ...
          <button 
            ...
            style={{ color: theme.textColor }}
          >
            <span className="hidden sm:inline font-mono text-sm uppercase tracking-wider">Next</span>
